...
var Authorizer = function(permissionService, organization_id, cb) {
  this.permissions = {};
  this.permissionService = permissionService;
	// Get permissions for entity
	this.permissionService.getPermissionsForEntity('organization', organization_id, function(err, permissions) {
		if(err) {
...
module.exports = function(req, res, next) {
  var authorizer = new Authorizer(req.permissions, req.session.organization_id, function(err) {
    if(err) {
      return next(err);
    }
